...
const SUB_PAGE_URL = "https://foolvpn.me/nautica"; // tidak dipakai lagi untuk redirect
const KV_PRX_URL = "https://raw.githubusercontent.com/wafidvpn/wafid/refs/heads/main/kvProxyList.json";
  ...
  if (prxBank.status == 200) {
    ...
    cachedPrxList = prxString
      .map((entry) => {
        ...
        return { prxIP: prxIP || "Unknown", prxPort: prxPort || "Unknown", country: country || "Unknown", org: org || "Unknown Org" };
      })
      .filter(Boolean);
  }
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/sub")) {
        return Response.redirect(`https://${APP_DOMAIN}`, 301);
        ...
        // tetap sama dengan file asli (handle /sub, /myip, dll.)
        // kode lengkap dari API v1 bagian ini tidak dihapus
      }
...
// --- sisanya tetap sama: websocketHandler, protocolSniffer, handleTCPOutBound, handleUDPOutbound, makeReadableWebSocketStream, readSsHeader, readFlashHeader, readHorseHeader, remoteSocketToWS, safeCloseWebSocket, checkPrxHealth, base64ToArrayBuffer, arrayBufferToHex, shuffleArray, reverse, getFlagEmoji ---
