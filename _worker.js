...
// Variables
const rootDomain = "wafidvpn.workers.dev"; // Ganti dengan domain utama kalian
const serviceName = "wafid05"; // Ganti dengan nama workers kalian
const apiKey = ""; // Ganti dengan Global API key kalian (https://dash.cloudflare.com/profile/api-tokens)
const apiEmail = "wafidvpn@gmail.com"; // Ganti dengan email yang kalian gunakan
...
async function getProxyList(proxyBankUrl = PROXY_BANK_URL) {
   ...
   * # Baris yang diawali tanda pagar (#) dianggap komentar dan diabaikan
   */
  if (!proxyBankUrl) {
  ...
  if (proxyBank.status == 200) {
    ...
    const proxyString = text
      ...
      .filter(line => line && !line.startsWith("#"));  // buang baris kosong & komentar
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    document.setTitle("Welcome to <span class='text-blue-500 font-semibold'>Latif.Dev.Id Server Proxy ID & SG List</span>");
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // Handle proxy client - WEBSOCKET HARUS DIPRIORITASKAN
      ...
      //  Lanjutkan dengan routing lainnya
